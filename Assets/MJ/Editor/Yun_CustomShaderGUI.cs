...
namespace Yun_FBShaderGUI
{
    ...
    /// 局内英雄面板
    /// </summary>
    public class Yun_CustomShaderGUI : Yun_CommonShaderGUI
    {
        ...
        static string[] _shadowTypes;

        static string[] shadowTypes
        {
            get
            {
                if (_shadowTypes == null)
                {
                    _shadowTypes = new string[] { "空", "高清"};
                }
                return _shadowTypes;
            }
        }
        ...
        void DrawSkinGUI(MaterialEditor materialEditor, Material material)
        {
            ...
            //阴影选择
            MaterialProperty shadowType = FindMaterialProperty("_ShadowType");
            int shadowTypeValue = (int)shadowType.floatValue;
            shadowTypeValue = EditorGUILayout.Popup(shadowTypeValue, shadowTypes);

            if (shadowTypeValue==0)
            {
                material.DisableKeyword(string.Intern("ENABLE_HQ_SHADOW"));
                material.DisableKeyword(string.Intern("ENABLE_HQ_AND_UNITY_SHADOW"));
                material.SetFloat("_ShadowType",0);
            }
            else
            {
                material.EnableKeyword(string.Intern("ENABLE_HQ_SHADOW"));
                material.DisableKeyword(string.Intern("ENABLE_HQ_AND_UNITY_SHADOW"));
                material.SetFloat("_ShadowType", 1);
            }
