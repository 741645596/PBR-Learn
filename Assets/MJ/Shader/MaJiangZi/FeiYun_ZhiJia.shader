Shader "FeiYun/Role/ZhiJia"
{
    Properties
    {

        [IntRange]_Row("行",Range(1,4)) = 0
        ...
        _LobeMix("皮肤光滑度权重",Range( 0 , 5)) = 1

        _EnvDiffInt("环境补光强度", Range(1, 8)) = 1
    ...
    SubShader
    {
        ...
        Pass  //SGameSkin
        {
            ...
            Varyings vert( Attributes v)
            {
                ...
                float u = v.uv.x;
                // float vm = v.uv.y - 0.25 * _Row;
                float vm = v.uv.y -floor(_Row) / 4.0;
    ...
    CustomEditor "FBShaderGUI.FeiYunZhiJiaSkinGUI"
}
