Shader "FB/UI/SGameUIUber_Stencil"
{
    Properties
    {
        ...
        [Enum(R,0,G,1,B,2,A,3)]_AlphaChannel("AlphaChannel", Float) = 0
        ...
        [Enum(Alpha Blend,0,Additive,1,Additive2,2,Soft Additive,3,Particle Additive,4,Premultiplied,5,UI,6)]_BlendMode("BlendMode", Float) = 0
        [Enum(Transparent,0,AlphaTest,1)]_SurfaceType("SurfaceType", Float) = 0
        [Toggle] _CloseLinearToSRGB("LinearToSRGB", Float) = 0
        ...
        [HideInInspector]_AlphaEnhance("透明度增强", Range(0,2)) = 1
    }
    ...
    struct Varyings
    {
        ...
        {
            half4 dissolve_tex = tex2D(_DissolveTex,uv);
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(Varyings i,half facing : VFACE):SV_TARGET
            {
                // Distort
                #if defined(_DISTORT_ON)
                    half4 distort_tex = tex2D(_DistortionTex,i.uv1.xy);
                    half distort = 0;
                ...
                #endif 
                main_uv = lerp(main_uv,clamp(main_uv,0,1),half2(_MainUVXClamp,_MainUVYClamp));  // Clamp

                float4 final_color = tex2D(_MainTex,main_uv) * i.vertexColor;
                half alpha = final_color.a;
                ...
                // Alpha
                    ...
                    #endif 
                    alpha_uv = lerp(alpha_uv,clamp(alpha_uv,0,1),half2(_AlphaUVXClamp,_AlphaUVYClamp));

                    half4 alpha_map = tex2D(_AlphaTex,alpha_uv);
                ...
                // Second Layer
                    ...
                    #endif 
                    second_uv = lerp(second_uv,clamp(second_uv,0,1),half2(_SecondUVXClamp,_SecondUVYClamp));
                    float4 second_color = tex2D(_SecondTex,second_uv) * _SecondColor;
                ...
                final_color.rgb = lerp(LinearToSRGB(final_color.rgb),final_color.rgb,_CloseLinearToSRGB);
                
                // Handle Blend One One
                half isOneOne = _BlendSrc == 1.0 && _BlendDes == 1.0;
                final_color.rgb = lerp(final_color.rgb , final_color.rgb * alpha,isOneOne);
