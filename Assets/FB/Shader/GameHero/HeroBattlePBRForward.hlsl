#ifndef HEROBATTLE_PBRFORWARD
#define HEROBATTLE_PBRFORWARD
...
struct VaryingsPBR
{
	...
	#if defined(_DISSOLVE_ON) // || defined(_LIGHT_TEX_ON)
		float4 uvDisMatCap                : TEXCOORD5;
	#endif
...
VaryingsPBR PassVertexPBR(AttributesSSS input)
{
	...
	UNITY_TRANSFER_INSTANCE_ID(input, output);
		...
		half3 viewDir = normalize(ObjSpaceViewDir(input.positionOS));
		half dotProduct = 1-dot(normalize(input.normalOS), viewDir);
		#if defined(_RIM_COLOR_ON)
			half4 rimColor = GET_PROP(_RimColor);
			output.color.rgb = (rimColor.rgb * pow(dotProduct, rimColor.w)).rgb;
			output.color.a=dotProduct;
		#endif
		#if defined(_HURT_EFFECT_ON)
			output.hurtColor=dotProduct*dotProduct*2;
		#endif
	#endif

	#if defined(_HURT_SHAKEEFFECT_ON)
	    float3 wsPos=TransformObjectToWorld(input.positionOS.xyz);
		wsPos=Shake(wsPos);
	#endif

	//uv
	output.uv.xy = UVTilingOffset(input.texcoord.xy,GET_PROP(_PBRBaseMapOffset));

	//
	#if defined(_HURT_SHAKEEFFECT_ON)
	    output.positionWS = wsPos;
	#else
	...
	#if defined(LIGHTMAP_ON)
		output.lightmapUV.xy = lightmapUV.xy * unity_LightmapST.xy + unity_LightmapST.zw;
	#else
		output.vertexSH.xyz = PBRSampleSHVertex(output.normalWS.xyz);
	#endif

	#if defined(_DISSOLVE_ON) 
		output.uvDisMatCap.xy = input.texcoord.xy + frac(GET_PROP(_ScrollSpeed).xy * _Time.x);
		output.uvDisMatCap.xy = UVTilingOffset(output.uvDisMatCap.xy,_DissolveTex_ST);
	#endif

	#if defined(_DISSOLVE_ON) // ||defined(_LIGHT_TEX_ON)
		output.uvDisMatCap.z = dot(UNITY_MATRIX_IT_MV[0].xyz, input.normalOS) * 0.5 + 0.5;
		output.uvDisMatCap.w = dot(UNITY_MATRIX_IT_MV[1].xyz, input.normalOS) * 0.5 + 0.5;
	#endif
...
inline void InitSurfaceDataPBR(VaryingsPBR input, out SurfaceDataPBR outSurfaceData)
{
	...
	half4 pbrBaseColor = GET_PROP(_PBRBaseColor);
	//alpha
	...
	outSurfaceData.smoothness = specGloss.a *UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, GET_PROP(_Smoothness));
	outSurfaceData.metallic = specGloss.r * UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, GET_PROP(_Metallic));

	//normalTS
	outSurfaceData.normalTS = UnpackNormalScale(SAMPLE_TEXTURE2D(_BumpMap, sampler_BumpMap, input.uv.xy), UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, GET_PROP(_NormalScale)));
	//outSurfaceData.normalTS = UnpackNormalScale(SAMPLE_TEXTURE2D(_BumpMap, sampler_BumpMap, input.uv.xy), GET_PROP(_NormalScale));

	//ao
	outSurfaceData.occlusion = PBRLerpWhiteTo(specGloss.g, UNITY_ACCESS_INSTANCED_PROP(UnityPerMaterial, GET_PROP(_OcclusionStrength)));

	//emission
	half4 emissionTex = SAMPLE_TEXTURE2D(_EmissionMap, sampler_EmissionMap, input.uv.xy) * GET_PROP(_EmissionColor);
