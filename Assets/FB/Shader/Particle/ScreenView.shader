Shader "FB/Particle/ScreenView"
{
    Properties
    {
        ...
        [Space(25)]
        [MaterialToggle(_VIEWUV_ON)]_ViewUV("UV是否跟着相机转", int) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
                ...
                float3 pos_world_center = TransformObjectToWorld(float3(0,0,0));
                v.uv = pos_world.xy - pos_world_center.xy;

#ifdef _VIEWUV_ON 
                float3 pos_view = TransformWorldToView(pos_world);
                float3 pos_view_center = TransformWorldToView(pos_world_center);
                v.uv = pos_view.xy - pos_view_center.xy;
#endif
