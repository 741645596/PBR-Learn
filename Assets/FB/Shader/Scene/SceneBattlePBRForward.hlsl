...
#include "SceneBattlePBRInput.hlsl"
...
struct VaryingsPBR
{
	...
	float3 viewDirWS                : TEXCOORD4;
		...
		float4 waveUVA : TEXCOORD5;
		float4 waveUVB : TEXCOORD6;
	#endif
...
VaryingsPBR PassVertexPBR(AttributesSSS input)
{
	...
	#if defined(_MATTYPE_RIVER)
	    output.waveUVA.xy = TRANSFORM_TEX(input.lightmapUV.xy, _WaveMaskTex);
	    output.waveUVB.xy = TRANSFORM_TEX(input.lightmapUV.xy, _WaveNoiseTex)+ frac(_WaveSpeed.xy * _Time.x);
		output.waveUVB.zw = TRANSFORM_TEX(input.lightmapUV.xy, _WaveNoiseTex)+ frac(_WaveSpeed.zw * _Time.x);
	#endif
	...
	output.normalWS = TransformObjectToWorldNormal(input.normalOS);
		...
		float3 vPos = output.positionWS.xyz;
		float time = _Time.y * 0.5;
		GrassVertexApplyBlending(vPos, output.normalWS.xyz, input.color, time, _BendScale, _BranchAmp, _DetailFreq, _DetailAmp);
		output.positionWS.xyz.xyz=vPos;
		//互动
		float3 dir = output.positionWS.xyz-_PlayerPos.xyz;
		float dis = length(dir)/_PlayerPos.w;
		dis=clamp(dis,0,1);
		dir=normalize(dir);
		float lerpY=(output.positionWS.y-_PlayerPos.y)/_PlayerPos.w;
		lerpY=clamp(lerpY,0,1)*2;
		dir.y=dir.y*0.5;
		output.positionWS.xyz=output.positionWS.xyz+dir*lerp(_PlayerPos.w,0,dis)*lerpY;
	#endif
