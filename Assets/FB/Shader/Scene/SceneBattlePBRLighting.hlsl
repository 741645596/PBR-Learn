...
#include "SceneBattlePBRForward.hlsl"
...
half4 FragmentRES(VaryingsPBR input){
    ...
    SurfaceDataPBR surfaceData;
        ...
        half4 waveMaskTex = SAMPLE_TEXTURE2D(_WaveMaskTex, sampler_WaveMaskTex, input.waveUVA.xy);
		half4 waveNoiseTexA = SAMPLE_TEXTURE2D(_WaveNoiseTex, sampler_WaveNoiseTex, input.waveUVB.xy);
		half4 waveNoiseTexB = SAMPLE_TEXTURE2D(_WaveNoiseTex, sampler_WaveNoiseTex, input.waveUVB.zw);
		half2 uv1 = waveNoiseTexA.b * waveMaskTex.r;
		input.uv.xy=input.uv.xy+uv1*_WaveStrength.x;
        //映射
    ...
    #if defined(_MATTYPE_RIVER)
        surfaceData.albedo.rgb=lerp(surfaceData.albedo.rgb,waveNoiseTexB.rgb,waveMaskTex.r*_WaveStrength.y)+waveLerp;
    #endif
