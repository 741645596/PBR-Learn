Shader "FB/UnityShader/Hair"
{
    Properties{
        ...
        _OffsetInt("位移缩放", vector) = (10.0,1.0,0.0,0.0)
        _SpecularInt("高光亮度", Range(0, 1)) = 0.3
        _SpecularRange("高光聚集", Range(0, 1)) = 0.3
        _AnisotropyInt("锯齿强度", Range(-1,1)) = 0.5
        _UnderSpecular("底层提亮", Range(0,1)) = 0.2
    ...
    SubShader
    {
        ...
        pass
        {
            ...
            half4 frag(v2f i) :SV_TARGET
            {
                ...
                float3 n1DirWS = normalize(nDirWS * Var_offstTex + i.bDirWS);
                ...
                half3 var_BaseMap = tex2D(_BaseMap,i.uv0) * _BaseColor;
                ...
                // Mix Color
                half3 color = var_BaseMap * min(1.2, max(nDotl + max(0, specular3 * _UnderSpecular), 0.2));
